...
function renderTable() {
  ...
  if (currentTab === 'methods') {
    ...
    var rows = pageData.map(function(c) {
      ...
      return '<tr><td><span class="avatar-sm">' + c.name.charAt(0) + '</span></td><td>' + c.name + '</td><td>' + c.method + '</td><td style="font-family:monospace;font-size:11px;color:#6B7280">' + c.code + '</td>' + statusHtml + '<td class="action-cell"><div class="action-inner">' + UI.btn.icon('edit', "openChannelModal('" + c.id + "')", '編輯') + '</div></td></tr>';
    }).join('');
